...
void demo(char *cfgfile, char *weightfile, float thresh, int cam_index, const char *filename, char **names, int classes, int frame_skip, char *prefix, char *out_filename)
{
    ...
    if(filename)
    {
        char* named=malloc(strlen(filename)+strlen("predictions/result_.avi"));
        sprintf(named,"%s",filename);
        sprintf(named,"predictions/result_%s.avi",strsep(&named,"."));
        out_filename=named;
    }
    else
    {
        char* named=malloc(strlen("predictions/result_feed.avi"));
        sprintf(named,"predictions/result_feed.avi");
        out_filename=named;
    }
    if(filename) {
        printf("video file: %s\n", filename);
    ...
    if(!prefix) {
        ...
        cvResizeWindow(filename, 1352, 1013);
    }

    double before   = get_wall_time();
    g_before = (double)time(NULL);
    while(1) {
        ++count;
        if(1) {
            ...
            if(!prefix) {
                show_image_cv_ipl(show_img, filename, out_filename);
                int c = cvWaitKey(1);
                if (c == 10) {
